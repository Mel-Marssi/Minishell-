...
/*   Updated: 2023/03/17 17:32:13 by mel-mars         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **env)
{
	...
	while (argc && argv)
	{
		(tcsetattr(0, 0, &(&w)->new), signal(SIGINT, signal_handler), signal(SIGQUIT, SIG_IGN));
		w.count = 0;
		w.h = NULL;
		w.str = readline("Minishell $> ");
		(signal(SIGINT, signal_c),signal(SIGQUIT, signal_s), add_h(w.str));
		if (w.str == NULL)
			(ft_printf("exit\n"), exit(g_exit_status));
